...
//import com.kingscripts.autoskyblock.autoskyblockmod.utils.RoomDetectionUtils;
...
//
//import static com.kingscripts.autoskyblock.autoskyblockmod.macros.dungeons.catacombs.DungeonManager.entranceMapCorners;
//import static com.kingscripts.autoskyblock.autoskyblockmod.macros.dungeons.catacombs.DungeonManager.gameStage;
//
//import static com.kingscripts.autoskyblock.autoskyblockmod.macros.dungeons.catacombs.DungeonManager.*;
//
...
//                    AutoSkyblock.logger.debug("DungeonRooms: New Single Thread Executor Started");
//                }
...
//                    AutoSkyblock.logger.info("DungeonRooms: Getting entrance map corners from hotbar map...");
//                } else if (entranceMapCorners[0] == null || entranceMapCorners[1] == null) { //prevent crashes if hotbar map bugged
//                    AutoSkyblock.logger.warn("DungeonRooms: Entrance room not found, hotbar map possibly bugged");
...
//                        AutoSkyblock.textToDisplay = new ArrayList<>(Collections.singletonList(
...
//                    AutoSkyblock.logger.warn("DungeonRooms: Entrance Room coordinates not found");
...
//                            }
//                        } else {
//                            AutoSkyblock.textToDisplay = new ArrayList<>(Arrays.asList(
...
//                        incompleteScan = 0;
//                        AutoSkyblock.logger.info("DungeonRooms: Rescanning room...");
//                        raytraceBlocks();
//                    } else if (redoScan != 0 && System.currentTimeMillis() > redoScan) {
//                        redoScan = 0;
//                        AutoSkyblock.logger.info("DungeonRooms: Clearing data and rescanning room...");
//                        possibleRooms = null;
//                        raytraceBlocks();
//                    }
//
...
//                            AutoSkyblock.textToDisplay = new ArrayList<>(Collections.singletonList("Dungeon Rooms: "
...
//                        AutoSkyblock.logger.info("DungeonRooms: 576 raytrace vectors sent, returning "
...
//                        ));
//
//                        AutoSkyblock.logger.debug("DungeonRooms: Possible rooms list = " + new ArrayList<>(possibleRoomsSet));
...
//        AutoSkyblock.textToDisplay = null;
...
//            if (AutoSkyblock.roomsJson.get(roomName) != null) {
//                Waypoints.secretNum = AutoSkyblock.roomsJson.get(roomName).getAsJsonObject().get("secrets").getAsInt();
...
//
//                if (AutoSkyblock.roomsJson.get(roomName) != null) {
//                    JsonObject roomJson = AutoSkyblock.roomsJson.get(roomName).getAsJsonObject();
//                    if (roomJson.get("fairysoul").getAsBoolean()) {
//                        line = line + EnumChatFormatting.WHITE + " - " + EnumChatFormatting.LIGHT_PURPLE + "Fairy Soul";
//                    }
//                    lineList.add(line);
//
//                    if (Waypoints.enabled && roomJson.get("secrets").getAsInt() != 0 && AutoSkyblock.waypointsJson.get(roomName) == null) {
//                        lineList.add(EnumChatFormatting.RED + "No waypoints available");
//                        lineList.add(EnumChatFormatting.RED +  "Press \"" + GameSettings.getKeyDisplayString(AutoSkyblock.keyBindings[0].getKeyCode()) +"\" to view images");
//                    }
//                }
//                AutoSkyblock.textToDisplay = lineList;
//            }
//        }
//    }
//
//
//    void raytraceBlocks() {
//        AutoSkyblock.logger.debug("DungeonRooms: Raytracing visible blocks");
...
//            }
//        }
//        AutoSkyblock.logger.debug("DungeonRooms: Finished raytracing, amount of blocks to check = " + blocksToCheck.size());
//        long timeFinish = System.currentTimeMillis();
//        AutoSkyblock.logger.debug("DungeonRooms: Time to raytrace and filter (in ms): " + (timeFinish - timeStart));
//
...
//            AutoSkyblock.logger.debug("DungeonRooms: Initializing Room Comparison Executor");
...
//                AutoSkyblock.logger.debug("DungeonRooms: No previous possible rooms list, creating new list...");
...
//                    updatedPossibleRooms.put(direction, new ArrayList<>(AutoSkyblock.ROOM_DATA.get(roomCategory).keySet()));
//                }
//            } else {
//                //load info from previous scan
//                AutoSkyblock.logger.debug("DungeonRooms: Loading possible rooms from previous room scan...");
...
//            }
//
//            AutoSkyblock.logger.debug("DungeonRooms: directionCorners " + directionCorners.entrySet());
//
...
//                AutoSkyblock.logger.debug("DungeonRooms: BlockPos being checked " + blockPos);
...
//                        int index = Arrays.binarySearch(AutoSkyblock.ROOM_DATA.get(roomCategory).get(roomName), idToCheck);
//                        if (index > -1) {
...
//
//                    AutoSkyblock.logger.debug("DungeonRooms: direction checked = " + direction + ", longID = " + idToCheck + ", relative = " + relative);
//                    AutoSkyblock.logger.debug("DungeonRooms: updatedPossibleRooms size = " + updatedPossibleRooms.get(direction).size() + " for direction " + direction);
//                }
//                blocksChecked.add(blockPos);
//
//                if (combinedMatchingRooms == 0) {
//                    AutoSkyblock.logger.warn("DungeonRooms: No rooms match the input blocks after checking " + blocksChecked.size() + " blocks, returning");
//                    break;
//                }
//                if (combinedMatchingRooms == 1) {
//                    //scan 10 more blocks after 1 room remaining to double check
//                    if (doubleCheckedBlocks >= 10) {
//                        AutoSkyblock.logger.debug("DungeonRooms: One room matches after checking " + blocksChecked.size() + " blocks");
//                        break;
//                    }
//                    doubleCheckedBlocks++;
//                }
//
//                AutoSkyblock.logger.debug("DungeonRooms: " + combinedMatchingRooms + " possible rooms after checking " + blocksChecked.size() + " blocks");
//
//            }
//
//            if (blocksChecked.size() == blocksToCheck.size()) { //only print for this condition bc other conditions break to here
//                AutoSkyblock.logger.warn("DungeonRooms: Multiple rooms match after checking all " + blocksChecked.size() + " blocks");
//            }
//
...
//
//            long timeFinish = System.currentTimeMillis();
//            AutoSkyblock.logger.debug("DungeonRooms: Time to check blocks using thread (in ms): " + (timeFinish - timeStart));
//
