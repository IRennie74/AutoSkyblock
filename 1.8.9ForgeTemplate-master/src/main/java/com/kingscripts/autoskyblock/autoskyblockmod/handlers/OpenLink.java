...
public class OpenLink {
    ...
    public static void checkForLink(String type){
        ...
        if (RoomDetection.roomName.equals("undefined") || AutoSkyblock.roomsJson.get(RoomDetection.roomName) == null) {
            player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                    + "Dungeon Rooms: You do not appear to be in a detected Dungeon room right now."));
            return;
        }

        JsonObject roomJson = AutoSkyblock.roomsJson.get(RoomDetection.roomName).getAsJsonObject();
...
//                mc.addScheduledTask(() -> mc.displayGuiScreen(new LinkGUI()));
            ...
            case "dsg":
                ...
                break;
            case "sbp":
                if (AutoSkyblock.usingSBPSecrets) {
